...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.nested-children.update", [$nestedChild->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('children') ? 'is-invalid' : '' }}" name="children_id" id="children_id" required>
                    @foreach($childrens as $id => $entry)
                        <option value="{{ $id }}" {{ (old('children_id') ? old('children_id') : $nestedChild->children->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $nestedChild->name) }}">
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('age') ? 'is-invalid' : '' }}" type="number" name="age" id="age" value="{{ old('age', $nestedChild->age) }}" step="1">
                @if($errors->has('age'))
                    <div class="invalid-feedback">
                        {{ $errors->first('age') }}
                    </div>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\NestedChild::STUDY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('study') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="study_{{ $key }}" name="study" value="{{ $key }}" {{ old('study', $nestedChild->study) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('study_stage') ? 'is-invalid' : '' }}" type="text" name="study_stage" id="study_stage" value="{{ old('study_stage', $nestedChild->study_stage) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('study_place') ? 'is-invalid' : '' }}" type="text" name="study_place" id="study_place" value="{{ old('study_place', $nestedChild->study_place) }}">
